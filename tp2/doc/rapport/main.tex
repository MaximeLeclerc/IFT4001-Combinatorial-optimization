...
\begin{document}
	...
	\section{Description du problème}
		...
		Par exemple, en prenant le message crypté LXFOPVEFRNHR, le solveur doit trouver que la clé la plus probable est LEMON et que le texte original est ATTACKATDAWN. L'implémentation utilisée utilisera les fréquences des lettres trouvées d'une langue donnée.\footnote{Wikipedia, [En ligne]. \url{https://en.wikipedia.org/wiki/Letter\_frequency} (Page consultée le 3 avril 2016)} Cette implémentation sera donc plus efficace si la chaine est longue et si elle utilise des vrais mots de la langue choisie.\\
	...
	\section{Approche proposée}
		...
		\subsection{Amélioration des heuristiques}
			...
			\subsubsection{Stratégie pour choisir la variable}
				L'heuristique se construit avec le message chiffré $C$ et la longueur de la clé à trouver $k$ comme paramètres.
				...
				De cette façon, la variable associée au caractère le plus fréquent est aussi celle qui assigne le plus de variables à une valeur (toutes les autres occurrences du caractère).
				...
				On trouve alors que le caractère le plus présent est 'B' avec 4 occurrences.
				...
				La liste finale est donc: [1,0,5] qui représente: ['B','A','C'] \\
				...
				Pour $S_1$, le caractère le plus fréquent est 'A' avec une $3$ occurrence. \\
				Pour $S_2$, il s'agit plutôt de 'C' avec 3 occurrences.
				...
				En cas, d'égalité, on choisi le premier caractère trouvé, soit 'A' dans cet exemple.
				On ajoute à la liste le premier indice de ce caractère dans la chaîne de départ.
				Après toutes les itérations on obtient [0,5,4,1] qui représente: [ 'A', 'C', 'B' de $S_1$, 'B' de $S_2$]
			...
			\subsubsection{Stratégie pour choisir la valeur}
				Nous avons déterminé les lettres les plus utilité pour un langage.
				Par exemple, pour l'anglais, on a dans l'ordre les lettres suivantes: \\
				e,t,a,o,i,n,s,h,r,d,l,c,u,m,w,f,g,y,p,b,v,k,j,x,q,z \\
				Cette information est passée en paramètre pour construire la stratégie.
				Elle s'en servira pour créer une liste avec les positions de chaque lettre dans l'alphabet dans le même ordre.
